...
CHANGELOG = [
    {
        ...
        "highlights": [
            ...
            "Fix: same dev-debug toast cleanup in Pager (POCSAG/FLEX) and Morse modes — both used to push a raw 'Command: rtl_fm ...' / '[cmd] rtl_sdr ...' info card to the UI on every Start. Now logged to debug only.",
            "Fix: front-end SSE handlers for Sensor, Meters, Pager, and WiFi modes no longer surface every non-JSON decoder stdout line as a blue info toast (type:'raw'). Stack-trace fragments and banner chatter were leaking through as toasts like 'C:\\Users\\bemasher\\Desktop\\Go\\src\\github.com\\bemasher\\'. Raw lines now go to console.debug only; the {type:'info'} channel is reserved for actual user-facing messages.",
            "Fix: remove the verbose 'Sensor stream connected...' / 'RTLAMR stream connected...' / 'Stream connected...' toasts that fired on every SSE reconnect — they're implementation details the user doesn't need to see (the Start button transition and the decoder's own 'started' status are the real signal).",
        ],
    },
    {
        "version": "2.27.0-d",
